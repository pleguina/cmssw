...
StubsSimHitsMatcher::StubsSimHitsMatcher(const edm::ParameterSet& edmCfg, const OMTFConfiguration* omtfConfig): omtfConfig(omtfConfig) {
  ...
  stubsInLayersCntByPdgId = subDir.make<TH2I>("stubsInLayersCntByPdgId", "stubsInLayersCntByPdgId;;OMTF layer;", 3, 0, 3, omtfConfig->nLayers(), -.5, omtfConfig->nLayers() - 0.5);
  stubsInLayersCntByPdgId->SetCanExtend(TH1::kAllAxes);
  
  firedLayersCntByPdgId = subDir.make<TH2I>("firedLayersCntByPdgId", "firedLayersCntByPdgId", 3, 0, 3, omtfConfig->nLayers(), -.5, omtfConfig->nLayers() - 0.5);
...
void StubsSimHitsMatcher::match(const edm::Event& iEvent, const l1t::RegionalMuonCand* omtfCand, const AlgoMuonPtr& procMuon, std::ostringstream& ostr) {
  ...
  if (procMuon->isValid() && omtfCand) {
      ...
      if (stub && gpResult.isLayerFired(iLogicLayer)) {
        ...
        case MuonSubdetId::RPC: {
          ...
          if(rpcDigiSimLinkDetSet != rpcDigiSimLinkHandle.product()->end() ) {
            ostr<<"rpcDigiSimLinkDetSet: detId "<<rpcDigiSimLinkDetSet->detId()<<" size "<<rpcDigiSimLinkDetSet->size()<<"\n";
            for(auto& rpcDigiSimLink : *rpcDigiSimLinkDetSet) {
              ...
              if( abs(stubGlobalPhi - simHitStripGlobalPhi) < 0.02) {
                auto matchedTrackInfo = matchedTrackInfos.insert( MatchedTrackInfo(rpcDigiSimLink.getEventId().event(), rpcDigiSimLink.getTrackId()));
            ...
            for(auto layer :  superlayer->layers()) {
              ...
              for(; dtDigiSimLink != dtDigiSimLinks.second; dtDigiSimLink++) {
                ...
                if( abs(stubGlobalPhi - digiWireGlobal.phi()) < 0.03) {
                  auto matchedTrackInfo = matchedTrackInfos.insert( MatchedTrackInfo(dtDigiSimLink->eventId().event(), dtDigiSimLink->SimTrackId()));
                  matchedTrackInfo.first->matchedDigiCnt.at(iLogicLayer)++;
                }
        ...
        case MuonSubdetId::CSC: {
          ...
          for(auto* layer : chamber->layers()) {
            ...
            if(cscDigiSimLinkDetSet != cscStripDigiSimLinkHandle.product()->end() ) {
              ostr<<"cscDigiSimLinkDetSet: detId "<<cscDigiSimLinkDetSet->detId()<<" "<<layer->id()<<" size "<<cscDigiSimLinkDetSet->size()<<"\n";
              for(auto& cscDigiSimLink : *cscDigiSimLinkDetSet) {
                ...
                if( abs(stubGlobalPhi - digiStripGlobalPhi) < 0.03) {
                  auto matchedTrackInfo = matchedTrackInfos.insert( MatchedTrackInfo(cscDigiSimLink.eventId().event(), cscDigiSimLink.SimTrackId()));
                  matchedTrackInfo.first->matchedDigiCnt.at(iLogicLayer)++;
                }
                ostr
    ...
    for(auto matchedTrackInfo : matchedTrackInfos) {
      ...
      for (auto& trackingParticle : *trackingParticleHandle.product() ) { //finding the trackingParticle corresponding to the matchedTrackInfo
        if(trackingParticle.eventId().event() == matchedTrackInfo.eventNum && trackingParticle.g4Tracks().at(0).trackId() == matchedTrackInfo.trackId) {
          allMatchedTracksPdgIds->Fill(to_string(trackingParticle.pdgId()).c_str(), 1);
          matchedPart = &trackingParticle;

          ostr <<"trackingParticle: pdgId "<<std::setw(3)<<trackingParticle.pdgId()
               <<" event "<<std::setw(4)<<trackingParticle.eventId().event()<<" trackId "<<std::setw(8)<<trackingParticle.g4Tracks().at(0).trackId()
          ...
          if(trackingParticle.parentVertex().isNonnull() ) {
            ostr<<"parentVertex Rho "<<trackingParticle.parentVertex()->position().Rho()<<" z "<<trackingParticle.parentVertex()->position().z()
                <<" R "<<trackingParticle.parentVertex()->position().R()
                <<" eta "<<trackingParticle.parentVertex()->position().eta()<<" phi "<<trackingParticle.parentVertex()->position().phi()<<std::endl;

            for(auto& parentTrack : trackingParticle.parentVertex()->sourceTracks() ) {
              ostr<<"parentTrack:      pdgId "<<std::setw(3)<<parentTrack->pdgId()
                  <<" event "<<std::setw(4)<<parentTrack->eventId().event()<<" trackId "<<std::setw(8)<<parentTrack->g4Tracks().at(0).trackId()
                  <<" pt "<<std::setw(9)<<parentTrack->pt()
                  <<" eta "<<std::setw(9)<<parentTrack->momentum().eta()<<" phi "<<std::setw(9)<<parentTrack->momentum().phi()
                  <<std::endl;
            }
          }
    ...
    if(bestMatchedPart) {
      ...
      ostr <<"bestMatchedPart: pdgId "<<std::setw(3)<<bestMatchedPart->pdgId()
           <<" event "<<std::setw(4)<<bestMatchedPart->eventId().event()<<" trackId "<<std::setw(8)<<bestMatchedPart->g4Tracks().at(0).trackId()<<"\n\n";
    }
  }
}
