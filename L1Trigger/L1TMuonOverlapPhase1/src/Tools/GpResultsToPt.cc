...
higerGps(gps.size(), nullptr)
{
  for(auto gp : gps) {
    ...
    for(int iLowerGp = iGP -1; iLowerGp >= 0; iLowerGp--) {
      if(omtfConfig->getPatternPtRange(iLowerGp).ptTo == omtfConfig->getPatternPtRange(iGP).ptFrom && gp->key().theCharge == gps.at(iLowerGp)->key().theCharge) {
    ...
    for(int iHigherGp = iGP +1; iHigherGp < (int)gps.size(); iHigherGp++) {
      if(omtfConfig->getPatternPtRange(iHigherGp).ptFrom == omtfConfig->getPatternPtRange(iGP).ptTo && gp->key().theCharge == gps.at(iHigherGp)->key().theCharge) {
    ...
    ostrTtle.str("");
    ostrTtle<<"PatNum "<<gp->key().theNumber<<" ptCode "<<gp->key().thePt<<" Pt "<<patternPt.ptFrom<<"-"<<patternPt.ptTo<<" GeV"<<" charge "<<gp->key().theCharge;
...
unsigned int GpResultsToPt::lutAddres(AlgoMuons::value_type& algoMuon, unsigned int& candProcIndx) {
  ...
  if(lowerGps[iGP]) {
    addressL = unsigned(algoMuon->getGoldenPatern()->getResults()[candProcIndx][iRefHit].getPdfSum() - lowerGps[iGP]->getResults()[candProcIndx][iRefHit].getPdfSum() ) >> bitShift; //
  }
  if(higerGps[iGP])
    addressR =  unsigned(algoMuon->getGoldenPatern()->getResults()[candProcIndx][iRefHit].getPdfSum() - higerGps[iGP]->getResults()[candProcIndx][iRefHit].getPdfSum() ) >> bitShift; //
...
void GpResultsToPt::updateStat(AlgoMuons::value_type& algoMuon, unsigned int& candProcIndx, double ptSim, int chargeSim) {
  unsigned int lutAdd = lutAddres(algoMuon, candProcIndx);
  ...
  if( (gp->key().theCharge == chargeSim) &&
      (omtfConfig->getPatternPtRange(lowerGps[iGP]->key().number()).ptFrom < ptSim) && (ptSim < omtfConfig->getPatternPtRange(higerGps[iGP]->key().number()).ptTo) ) {
