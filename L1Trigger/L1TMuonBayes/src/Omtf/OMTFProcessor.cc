  ...
  for(auto& myCand: algoCands) {
    ...
    if( this->myOmtfConfig->fwVersion() >= 6 ) { //TODO fix the fwVersion
      if (   static_cast<unsigned int>(myCand->getFiredLayerBits()) == std::bitset<18>("000000110000000001").to_ulong()
          ...
          || static_cast<unsigned int>(myCand->getFiredLayerBits()) == std::bitset<18>("000011010000000000").to_ulong()
         ...
         quality = 1;
             ...
             static_cast<unsigned int>(myCand->getFiredLayerBits()) == std::bitset<18>("000001010000001100").to_ulong()
