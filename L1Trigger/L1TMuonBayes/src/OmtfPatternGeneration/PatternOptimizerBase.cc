  ...
  for(auto& gp : goldenPatterns) {
    ...
    for(unsigned int iLayer = 0; iLayer < gp->getPdf().size(); ++iLayer) {
      for(unsigned int iRefLayer = 0; iRefLayer < gp->getPdf()[iLayer].size(); ++iRefLayer) {
		...
		<<" meanDistPhi "<<gp->meanDistPhiValue(iLayer, iRefLayer, 0)<<" distPhiBitShift "<<gp->getDistPhiBitShift(iLayer, iRefLayer); //"_Pt_"<<patternPt.ptFrom<<"_"<<patternPt.ptTo<<"_GeV
        ...
        for(unsigned int iPdf = 0; iPdf < gp->getPdf()[iLayer][iRefLayer].size(); iPdf++) {
          hist->Fill(iPdf, gp->pdfValue(iLayer, iRefLayer, iPdf));
        }
    ...
    for(unsigned int iRefLayer = 0; iRefLayer < gp->getPdf()[0].size(); ++iRefLayer) {
      ...
      if(gp->key().theCharge == -1) {
        classProbHists[2 * iRefLayer]->Fill(gp->key().theNumber, gp->pdfValue(refLayerLogicNumber, iRefLayer, iPdf));
      }
      else
        classProbHists[2 * iRefLayer +1]->Fill(gp->key().theNumber, gp->pdfValue(refLayerLogicNumber, iRefLayer, iPdf));
    }
  }
