...
MuTimingModule::MuTimingModule(const ProcConfigurationBase* config) : config(config), timigTo1_Beta(config->nLayers()) {
  for (unsigned int iLayer = 0; iLayer < timigTo1_Beta.size(); ++iLayer) {
    //TODO rolls, eta and bins can depend on layer
    timigTo1_Beta[iLayer].assign(
        rolls,
        std::vector<std::vector<std::vector<short> > >(
            etaBins, std::vector<std::vector<short> >(timingBins, std::vector<short>(betaBins))));
  }
}
...
void MuTimingModule::process(AlgoMuonBase* algoMuon) {
  ...
  for (auto& stubResult : algoMuon->getStubResults()) {
    ...
    for (unsigned int iOne_beta = 0; iOne_beta < betaBins; iOne_beta++) {
      ...
      //      LogTrace("l1tOmtfEventPrint")<<__FUNCTION__<<":"<<__LINE__<<" layer "<<layer<<" roll "<<roll<<" etaBin "<<etaBin<<" hitTiming "<<stubResult.getMuonStub()->timing<<" hitTimingBin "<<hitTimingBin<<" = one_beta "<<iOne_beta<<" logPVal "<<logPVal
  ...
  LogTrace("l1tOmtfEventPrint") << __FUNCTION__ << ":" << __LINE__ << " maxBin " << maxBin << " maxVal " << maxVal
                                << " beta " << beta << std::endl;
  if (maxVal >= 3) {
