...
PdfModuleWithStats::PdfModuleWithStats(TkMuBayesProcConfigPtr& config) : PdfModule(config), pdfHists(config->nLayers()) {
  ...
  for (unsigned int iLayer = 0; iLayer < coefficients.size(); ++iLayer) {
    for (unsigned int iEtaBin = 0; iEtaBin < coefficients[iLayer].size(); ++iEtaBin) {
      ...
      for (unsigned int iRefLayer = 0; iRefLayer < coefficients[iLayer][iEtaBin].size(); ++iRefLayer) {
        ...
        if (false)  //when the pdfs are generated
          pdfHists[iLayer][iEtaBin].emplace_back(subDir.make<TH2I>(name.str().c_str(),
                                                                   name.str().c_str(),
                                                                   config->nPtBins(),
                                                                   0,
                                                                   config->nPtBins(),
  ...
  sigmaFactor = 1.3;
  edm::LogImportant("l1tOmtfEventPrint") << __FUNCTION__ << ":" << __LINE__
                                         << " PdfModuleWithStats: sigmaFactor: " << sigmaFactor << std::endl;
}
    ...
    for (unsigned int iEtaBin = 0; iEtaBin < pdfHists[iLayer].size(); ++iEtaBin) {
      for (unsigned int iRefLayer = 0; iRefLayer < pdfHists[iLayer][iEtaBin].size(); ++iRefLayer) {
        ...
        for (int ptBin = 0; ptBin < pdfHist->GetXaxis()->GetNbins(); ++ptBin) {
          ...
          TH1D* pdfHistInPtBin = pdfHist->ProjectionY(ostr.str().c_str(), ptBin + 1, ptBin + 1);
          //+1 Because the bins in root hist are counted from 1
          ...
          if (pdfHistInPtBin->Integral() <= 0) {
            std::cout << __FUNCTION__ << ": " << __LINE__ << " iLayer " << iLayer << " iEtaBin " << iEtaBin
                      << " iRefLayer " << iRefLayer << " ptBin " << ptBin
                      << " - no entries, coefficients not calculated" << std::endl;
            continue;
          }
          ...
          //the maximum value tha the logPdf can have (n.b. logPdf = pdfMaxLogVal - log(pdfVal) * pdfMaxLogVal / minPlog)
          ...
          for (int iBinPdf = 0; iBinPdf < pdfHistInPtBin->GetXaxis()->GetNbins(); iBinPdf++) {
            ...
            if ((ptBin <= 16 && pdfVal < config->minPdfVal()) ||
                (ptBin > 16 && pdfVal < config->minPdfVal() * 2))  //removing points with small statistics - TODO tune
            {
          ...
          if (ptBin < 5 && notEmtyBins < 10) {
            std::cout << __FUNCTION__ << ": " << __LINE__ << " iLayer " << iLayer << " iEtaBin " << iEtaBin
                      << " iRefLayer " << iRefLayer << " ptBin " << ptBin
                      << " - notEmtyBins < 10, coefficients not calculated" << std::endl;
            continue;
          }
          ...
          if (sigma != 0) {
            ...
            std::cout << __FUNCTION__ << ": " << __LINE__ << " iLayer " << iLayer << " iEtaBin " << iEtaBin
                      << " iRefLayer " << iRefLayer << " ptBin " << ptBin << " sigma " << std::setw(10) << sigma
                      << " a " << std::setw(10) << a << " b " << std::setw(10) << b << " c " << std::setw(10) << c
                      << " sigmaFactor " << sigmaFactor << std::endl;
            ...
            if (a == 0) {
              c = 0;
              std::cout << __FUNCTION__ << ": " << __LINE__ << " iLayer " << iLayer << " iEtaBin " << iEtaBin
                        << " iRefLayer " << iRefLayer << " ptBin " << ptBin << " a is zero, setting c to 0 as well "
                        << std::endl;
            }
            ...
            std::cout << __FUNCTION__ << ": " << __LINE__ << " iLayer " << iLayer << " iEtaBin " << iEtaBin
                      << " iRefLayer " << iRefLayer << " ptBin " << ptBin << " sigma = 0 !!!!!!!!!!! mean = " << mean
                      << std::endl;
          }
    ...
    for (unsigned int iEtaBin = 0; iEtaBin < pdfHists[iLayer].size(); ++iEtaBin) {
      for (unsigned int iRefLayer = 0; iRefLayer < pdfHists[iLayer][iEtaBin].size(); ++iRefLayer) {
        ...
        for (int ptBin = 0; ptBin < pdfHist->GetXaxis()->GetNbins(); ++ptBin) {
          ...
          std::ostringstream ostr;
          ostr << pdfHist->GetName() << "_ptBin_" << ptBin;
          TH1D* pdfHistInPtBin = pdfHist->ProjectionY(
              ostr.str().c_str(), ptBin + 1, ptBin + 1);  //+1 Because the bins in root hist are counted from 1
          pdfHistInPtBin->SetTitle(ostr.str().c_str());
          ...
          try {
            ...
            if (fitStatus == 0) {
              ...
              std::cout << __FUNCTION__ << ": " << __LINE__ << " iLayer " << iLayer << " iEtaBin " << iEtaBin
                        << " iRefLayer " << iRefLayer << " ptBin " << ptBin << " a0 " << std::setw(10) << a0 << " a1 "
                        << std::setw(10) << a1 << " a2 " << std::setw(10) << a2 << std::endl;
              ...
              std::cout << __FUNCTION__ << ": " << __LINE__ << " iLayer " << iLayer << " iEtaBin " << iEtaBin
                        << " iRefLayer " << iRefLayer << " ptBin " << ptBin << " a " << std::setw(10) << a << " b "
                        << std::setw(10) << b << " c " << std::setw(10) << c << std::endl;
              ...
              std::cout << __FUNCTION__ << ": " << __LINE__ << " iLayer " << iLayer << " iEtaBin " << iEtaBin
                        << " iRefLayer " << iRefLayer << " ptBin " << ptBin << " - fit failed!!!!!!!!!!!!!!!!!! "
                        << std::endl;
            }
          } catch (std::exception& err) {
            std::cout << __FUNCTION__ << ": " << __LINE__ << " iLayer " << iLayer << " iEtaBin " << iEtaBin
                      << " iRefLayer " << iRefLayer << " ptBin " << ptBin << " - fit failed " << err.what()
                      << std::endl;
          }
