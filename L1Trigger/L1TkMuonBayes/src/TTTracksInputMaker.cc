...
TrackingTriggerTracks TTTracksInputMaker::loadTTTracks(const edm::Event& event,
                                                       int bx,
                                                       const edm::ParameterSet& edmCfg,
                                                       const ProcConfigurationBase* procConf) {
  ...
  if (ttTracksSource == SIM_TRACKS) {
    ...
    for (unsigned int iSimTrack = 0; iSimTrack != simTracksHandle->size(); iSimTrack++) {
      ...
      if (simTrackRef->eventId().bunchCrossing() == bx) {
        if ((abs(simTrackRef->type()) == 13 || abs(simTrackRef->type()) == 1000015) &&
            simTrackRef->momentum().pt() > 2.5) {  //TODO 1000015 is stau
          ...
          LogTrace("l1tOmtfEventPrint") << __FUNCTION__ << ":" << __LINE__ << " bx " << bx
                                        << " adding ttTrack from simTrack: sim.type() " << simTrackRef->type()
                                        << " genpartIndex " << simTrackRef->genpartIndex() << " Beta() "
                                        << simTrackRef->momentum().Beta() << " added track " << *ttTrack << std::endl;
        }
      }
    }
    ...
    for (unsigned int iTP = 0; iTP != trackingParticlesHandle->size(); iTP++) {
      ...
      if (trackingParticlePtr->eventId().bunchCrossing() == bx) {
        //to emulate the trigger rules we should process every track not only the muons!!!!
...
void TTTracksInputMaker::addTTTrack(TrackingTriggerTracks& ttTracks,
                                    std::shared_ptr<TrackingTriggerTrack>& ttTrack,
                                    const ProcConfigurationBase* procConf) {
  ...
  ttTrack->setPtBin(procConf->ptHwToPtBin(ttTrack->getPtHw()));
  //TODO when real hardware skale available, move to this function, and implement properly ptHwToPtBin
