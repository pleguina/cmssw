...
void PdfModule::init() {
  for (unsigned int iLayer = 0; iLayer < coefficients.size(); ++iLayer) {
    ...
    coefficients[iLayer].assign(
        etaBins,
        std::vector<std::vector<std::vector<int> > >(
            config->nRefLayers(), std::vector<std::vector<int> >(config->nPtBins(), std::vector<int>(3))));
...
void PdfModule::processStubs(const MuonStubsInput& muonStubs,
                             unsigned int layer,
