/*
 * TkMuonBayesTrack.h
 * The objects of this class are produced by the L1Trigger/L1TMuonOverlapPhase1/plugins/L1TkMuonBayesTrackProducer.h
 * It is a muon track formed by matching the tracking trigger tracks to the muon stubs (DT, CSC, RPC) .
 *  Created on: Mar 15, 2019
 *      Author: Karol Bunkowski kbunkow@cern.ch
 */

#ifndef DataFormats_L1Trigger_TkMuonBayesTrack_H_
#define DataFormats_L1Trigger_TkMuonBayesTrack_H_
...
namespace l1t {
  class TkMuonBayesTrack;
  typedef BXVector<TkMuonBayesTrack> TkMuonBayesTrackBxCollection;
  ...
  typedef std::vector<TkMuonBayesTrack> TkMuonBayesTrackCollection;

  class TkMuonBayesTrack : public L1Candidate {
  public:
    ...
    TkMuonBayesTrack(const LorentzVector& p4);

    TkMuonBayesTrack(const edm::Ptr<L1TTTrackType> ttTrackPtr);

    ~TkMuonBayesTrack() override;
    ...
    const boost::dynamic_bitset<> getFiredLayerBits(unsigned int layerCnt) const {
      return boost::dynamic_bitset<>(layerCnt, firedLayerBits);
    }
    ...
    const unsigned int getFiredLayerCnt() const { return this->firedLayerCnt; }

    int pdfSum() const { return pdfSum_; }

    void setPdfSum(int pdfSum = 0) { this->pdfSum_ = pdfSum; }

    const edm::Ptr<L1TTTrackType>& getTtTrackPtr() const { return ttTrackPtr; }

    const SimTrackRef& getSimTrackPtr() const { return simTrackRef; }

    void setSimTrackPtr(const SimTrackRef& simTrackRef) { this->simTrackRef = simTrackRef; }

    const edm::Ptr<TrackingParticle>& getTrackPartPtr() const { return trackPartPtr; }

    void setTrackPartPtr(const edm::Ptr<TrackingParticle>& trackPartPtr) { this->trackPartPtr = trackPartPtr; }
    ...
    void setCandidateType(CandidateType candidateType) { this->candidateType = candidateType; }

    float getBetaLikelihood() const { return betaLikelihood; }

    void setBetaLikelihood(float betaLikelihood = 0) { this->betaLikelihood = betaLikelihood; }
...
#endif /* DataFormats_L1Trigger_TkMuonBayesTrack_H_ */
